...
export default {
  ...
  mounted() {
    this.todoListData.forEach((task) => {
      this.checkedTasks[task.title] = false;
    });
  },
};
</script>
          ...
          <div class="d-sm-flex ga-6">
            <div class="ms-n2 mt-n2">
              <v-checkbox
                color="primary"
                hide-details
                v-model="checkedTasks[list.title]"
              ></v-checkbox>
            </div>
            <div>
              <div class="d-flex ga-3 align-center">
                <h3
                  class="text-h6"
                  :class="{ strikethrough: checkedTasks[list.title] }"
                >
                  {{ list.title }}
                </h3>
                <v-chip :color="list.badgebg" size="small">{{
                  list.status
                }}</v-chip>
              </div>
    ...
    <v-card>
      ...
      <v-card-text class="pb-0">
        <v-form>
          ...
          <v-select
            :items="['Sachin', 'Steve', 'Jack', 'Wiliam', 'Dona']"
            variant="outlined"
          ></v-select>
        </v-form>
      </v-card-text>
