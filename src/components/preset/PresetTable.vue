  ...
  methods: {
    async getPresets() {
      ...
      this.presets = response.data.map((preset) => {
        let available = this.checkStockInPreset(preset.presetDetails) && preset.presetDetails.length > 0;
        if (available) {
    ...
    async submitEditPart() {
      ...
      if (NumOfUse <= 0 || NumOfUse == null || !PresetDetailID || PresetDetailID == null) {
        Swal.fire({
          icon: "error",
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          try {
            const response = await serverService.updatePresetDetailByID(PresetDetailID, { NumOfUse: parseInt(NumOfUse) });
    ...
    async submitAddPartToPreset() {
      ...
      this.addPartDataSet.forEach((item) => {
        if (!item.hasOwnProperty("NumOfUse") || item.NumOfUse == "" || item.NumOfUse == null) {
          foundProblem++;
        }
      });
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          try {
            ...
            const response = await serverService.bulkCreatePresetDetail(payload);
    ...
    async deletePartInPreset(item) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          try {
            const response = await serverService.deletePresetDetailByID(item.PresetDetailID);
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          Swal.fire({
            ...
            if (result.isConfirmed) {
              try {
                const response = await serverService.deletePresetByID(item.PresetID);
  ...
  <div>
    <v-row class="mb-3">
      ...
      <v-col cols="12" md="4" sm="6">
        <div class="pa-6 d-flex ga-3 align-center cursor-pointer rounded-lg bg-lightinfo" @click="getPresets">
          <div class="d-flex ga-3 align-center">
            ...
            <div>
              ...
              <p class="text-14 lh-normal textSecondary" v-if="presets.length == 0">0 Preset</p>
              <p class="text-14 lh-normal textSecondary" v-else>{{ presets.length }} Presets</p>
            </div>
          </div>
        </div>
      </v-col>
      <!-- active preset data -->
      <v-col cols="12" md="4" sm="6">
        <div class="pa-6 d-flex ga-3 align-center cursor-pointer rounded-lg bg-lightsuccess" @click="setPresetDataStatus(true)">
          <div class="d-flex ga-3 align-center">
            ...
            <div>
              ...
              <p class="text-14 lh-normal textSecondary" v-if="sumActive == 0">0 Preset</p>
              <p class="text-14 lh-normal textSecondary" v-else>{{ sumActive }} Presets</p>
            </div>
          </div>
        </div>
      </v-col>
      <v-col cols="12" md="4" sm="6">
        <!-- inactive data -->
        <div class="pa-6 d-flex ga-3 align-center cursor-pointer rounded-lg bg-lighterror" @click="setPresetDataStatus(false)">
          <div class="d-flex ga-3 align-center">
            ...
            <div>
              ...
              <p class="text-14 lh-normal textSecondary" v-if="sumInActive == 0">0 Preset</p>
              <p class="text-14 lh-normal textSecondary" v-else>{{ sumInActive }} Presets</p>
            </div>
          </div>
        </div>
      </v-col>
    ...
    <v-row>
      ...
      <v-col cols="12" md="3">
        <v-btn height="48" block color="secondary" variant="flat" dark @click="openAddPresetDialog"
    ...
    <v-data-table class="border rounded-md" :search="search" :headers="headers" :items="presets" v-if="presets.length > 0">
      <template #[`item.PresetID`]="{ item }"> #{{ item.PresetID }} </template>

      <template #[`item.Status`]="{ item }">
        <div v-if="item.presetDetails" class="d-flex justify-center">
          <v-chip :color="item.available ? 'success' : 'error'" style="width: 100px" size="small" label class="d-flex justify-center align-center">
            {{ item.available ? "พร้อมใช้งาน" : "ไม่พร้อมใช้งาน" }}
          </v-chip>
        </div>
      </template>

      <template #[`item.ref_model_category_parts`]="{ item }">
        <div v-if="item.ref_model_category_parts" class="d-flex justify-end">
          <!-- {{ item.presetDetails.length }} ชิ้นส่วน -->

          <v-chip color="light" style="width: 100px" size="small" label class="d-flex justify-center align-center" link @click="openShowRefDialog(item)">
            {{ item.ref_model_category_parts.length }} กลุ่มงาน
            <v-tooltip activator="parent" location="bottom">ดูรายละเอียด</v-tooltip>
          </v-chip>
        </div>
      </template>

      <template #[`item.presetDetails`]="{ item }">
        <div v-if="item.presetDetails" class="d-flex justify-end">
          <!-- {{ item.presetDetails.length }} ชิ้นส่วน -->

          <v-chip color="info" style="width: 100px" size="small" label class="d-flex justify-center align-center" link @click="openPartDialog(item)">
            {{ item.presetDetails.length }} ชิ้นส่วน
            <v-tooltip activator="parent" location="bottom">ดูรายละเอียด</v-tooltip>
          </v-chip>
        </div>
    ...
    <!-- Dialog show Part -->
    <v-dialog v-model="dialogShowPart" class="dialog-mw" style="max-width: 1000px" persistent>
      <v-card class="pa-3" v-if="showPartData != null">
        <v-card-title>
          <v-row>
            <v-col cols="9">#{{ showPartData.PresetID }}. {{ showPartData.Preset }}</v-col>
            <v-col>
              <v-btn block color="secondary" variant="flat" dark @click="openAddPartToPresetDialog"
                ><v-icon size="20">mdi-plus-circle-outline</v-icon>
                <span class="hidden-sm-and-down">&nbsp;เพิ่มอุปกรณ์ในพรีเซ็ต</span>
              </v-btn>
            </v-col>
          </v-row>
        </v-card-title>
        <v-card-text>
          <v-data-table :headers="partHeaders" :items="showPartData.presetDetails" :row-props="setRowClass" density="compact">
            <template #[`item.part.PartID`]="{ index }">
              {{ index + 1 }}
            </template>
            <template #[`item.actions`]="{ item }">
              <v-btn color="primary" variant="outlined" size="small" class="mr-1" @click="openEditPartDialog(item)"><v-icon>mdi-pencil-outline</v-icon></v-btn>
              <v-btn color="error" variant="outlined" size="small" @click="deletePartInPreset(item)"><v-icon>mdi-delete-variant</v-icon></v-btn>
            </template>
          </v-data-table>
        </v-card-text>
        <!-- <v-card-actions>
        ...
        <v-card-actions>
          <v-btn color="error" @click="closePartDialog" block flat>ปิดหน้าต่าง</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    <!-- Dialog show Part -->
    ...
    <v-dialog v-model="dialogEditPart" class="dialog-mw" style="max-width: 500px" persistent>
      <v-card class="pa-3" v-if="editPart != null">
        <v-card-text>
          <v-text-field label="ชื่ออุปกรณ์" v-model="editPart.PartName_th" readonly />
          <v-text-field label="จำนวนที่ใช้" type="number" v-model="editPart.NumOfUse" />
        </v-card-text>
        <v-card-actions>
          <v-btn color="info" block flat @click="submitEditPart">บันทึกข้อมูล</v-btn>
        </v-card-actions>
        <v-card-actions>
          <v-btn color="error" @click="closeEditPartDialog" block flat>ปิดหน้าต่าง</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    <!-- Dialog edit Part -->
    ...
    <v-dialog v-model="dialogAddPreset" class="dialog-mw" style="max-width: 500px" persistent>
      <v-card class="pa-3">
        ...
        <v-card-actions>
          <v-btn color="info" block flat @click="submitAddPreset">บันทึกข้อมูล</v-btn>
        </v-card-actions>
        <v-card-actions>
          <v-btn color="error" @click="closeAddPresetDialog" block flat>ปิดหน้าต่าง</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    <!-- Dialog Add Preset -->
    <!-- Dialog Add Part To Preset -->
    <v-dialog v-model="dialogAddPartToPreset" class="dialog-mw" style="max-width: 1000px" persistent>
      <v-card class="pa-3">
        <v-card-text>
          <v-autocomplete
            color="primary"
          ...
          <v-data-table :items="addPartDataSet" :headers="addPartToPrasetHeader" density="compact" class="mt-3">
            <template #[`item.NumOfUse`]="{ item }">
              <div class="d-flex align-center justify-center">
                <v-text-field style="width: 50px" v-model="item.NumOfUse" label="จำนวน" type="number" hide-details />
              </div>
            </template>
          </v-data-table>
        </v-card-text>
        <v-card-actions>
          <v-btn color="info" block flat @click="submitAddPartToPreset">บันทึกข้อมูล</v-btn>
        </v-card-actions>
        <v-card-actions>
          <v-btn color="error" @click="closeAddPartToPresetDialog" block flat>ปิดหน้าต่าง</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    <!-- Dialog Add Part To Preset -->
    <!-- Dialog Show Ref -->
    <v-dialog v-model="dialogShowRef" class="dialog-mw" style="max-width: 1200px" persistent>
      <v-card class="pa-3">
        <v-card-text>
          <!-- <v-text-field label="ชื่อพรีเซ็ต" v-model="addPreset.Preset" /> -->
          <!-- <v-list-item
          ...
          <v-table class="border">
            ...
            <tbody>
              <tr v-if="showRef.length == 0">
                <td colspan="4" class="text-center">ไม่พบกลุ่มงานที่ใช้พรีเซ็ตนี้</td>
              </tr>
              <tr v-else v-for="(item, index) in showRef" :key="index">
                <td>{{ index + 1 }}.</td>
                <td>{{ item.partCategory.PartCategoryName }}</td>
                <td width="20%">{{ item.brand.Brand }} ({{ item.model.Model }})</td>
        ...
        <v-card-actions>
          <v-btn color="error" @click="closeShowRefDialog" block flat>ปิดหน้าต่าง</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    <!-- Dialog Show Ref -->
